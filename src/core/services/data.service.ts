...
export class DataService {
  ...
  private addContentType(options: DataServiceOptions): DataServiceOptions {
    if (options.method !== RequestMethod.Get) { // IF ADDED BY ME FOR TESTING...
  ...
  private interpolateUrl(options: DataServiceOptions): DataServiceOptions {
    ...
    options.url = options.url.slice(0, options.url.indexOf(separator) + separator.length)
      + options.url.slice(options.url.indexOf(separator) + separator.length).replace(/\/{2,}/g, '/');
